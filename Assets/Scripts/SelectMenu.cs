...
public class SelectMenu : MonoBehaviour, IPointerDownHandler
{
    ...
    IEnumerator Cam_ZoomIn() {
        while(true) {
            cam.transform.localPosition = Vector3.Slerp(cam.transform.localPosition, new Vector3(0, 2, -3.5f), 20 * Time.deltaTime);
    ...
    IEnumerator Cam_ZoomOut() {
        while(true) {
            cam.transform.localPosition = Vector3.Slerp(cam.transform.localPosition, new Vector3(0, 3, -5f), 20 * Time.deltaTime);
