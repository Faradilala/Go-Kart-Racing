...
public class Car2 : MonoBehaviour
{
    ...
    public void StartAI() {
        if(!player) {
            target = GameManager2.instance.target[nextTarget];
    ...
    IEnumerator AI_Move() {
        ...
        while(true) {
            ...
            if(dis <= 1) {
                nextTarget += 1;
                if(nextTarget >= GameManager2.instance.target.Length)
                    nextTarget = 0;
    ...
    private void OnTriggerEnter(Collider other) {
        if(player) {
            if(other.gameObject.tag == "Finish") {
                if(GameManager2.instance.check) {
                    GameManager2.instance.check = false;

                    if(GameManager2.instance.lap > 0) {
                        SE_Manager.instance.PlaySound(SE_Manager.instance.lap);
                        GameManager2.instance.LapTime();
                    }
                    GameManager2.instance.lap += 1;
                }
            }

            if(other.gameObject.tag == "CheckPoint") {
                GameManager2.instance.check = true;
            }
        }
    }
}