...
public class Car1 : MonoBehaviour
{
    ...
    public void StartAI() {
        if(!player) {
            target = GameManager1.instance.target[nextTarget];
    ...
    IEnumerator AI_Move() {
        ...
        while(true) {
            ...
            if(dis <= 1) {
                nextTarget += 1;
                if(nextTarget >= GameManager1.instance.target.Length)
                    nextTarget = 0;
    ...
    private void OnTriggerEnter(Collider other) {
        if(player) {
            if(other.gameObject.tag == "Finish") {
                if(GameManager1.instance.check) {
                    GameManager1.instance.check = false;

                    if(GameManager1.instance.lap > 0) {
                        SE_Manager.instance.PlaySound(SE_Manager.instance.lap);
                        GameManager1.instance.LapTime();
                    }
                    GameManager1.instance.lap += 1;
                }
            }

            if(other.gameObject.tag == "CheckPoint") {
                GameManager1.instance.check = true;
            }
        }
    }
}